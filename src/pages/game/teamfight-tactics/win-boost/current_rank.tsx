...
import current_lp from "../../../../data/game/team-fight-tactics/current_lp.json";
import servers from "../../../../data/game/team-fight-tactics/servers.json";
...
const CurrentRank = (props: any) => {
  ...
  // initilize variable
  useEffect(() => {
    setData(rank.filter((d: any) => !d.level || d.level < 4));
      ...
      <div className="border-t border-indigo-800 pt-4 mt-4">
            ...
            division.map((d: any, index: number) => (
              <button
                ...
                className={`px-5 py-2 rounded-lg ${
                  current_rank?.division?.value == d.value
                    ? "bg-indigo-800"
                    : "bg-indigo-950"
                } hover:bg-indigo-800 bg-opacity-70`}
        ...
        <div className="mt-6 flex flex-wrap justify-start gap-x-6 gap-y-4">
          {!current_rank?.rank?.level ? (
            ...
            <div className="flex flex-col gap-2">
              <label htmlFor="">{current_rank?.rank?.title} LP</label>
              <div className="">
                <Number
                  value={current_rank?.lp}
                  onChange={(newValue: number) => dispatch(setLP(newValue))}
                />
              </div>
            </div>
          )}
